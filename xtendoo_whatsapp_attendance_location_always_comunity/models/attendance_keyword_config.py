...
class AttendanceKeywordConfigLocationAlways(models.Model):
    _name = 'attendance.keyword.config.location.always'
    _description = 'Configuración de Palabras Clave para Asistencia WhatsApp con Ubicación Obligatoria'
    _rec_name = 'name'
    ...
    location_request_message = fields.Text(
        string='Mensaje Solicitud Ubicación',
        help='Mensaje para solicitar ubicación. Variables: {employee_name}, {action}'
    )

    location_received_message = fields.Text(
        string='Mensaje Ubicación Recibida',
        help='Mensaje cuando se recibe ubicación. Variables: {employee_name}, {latitude}, {longitude}, {maps_url}'
    )
    ...
    def get_location_request_message(self, employee_name, action):
        """Genera el mensaje de solicitud de ubicación"""
        if self.location_request_message:
            message = self.location_request_message
            message = message.replace('{employee_name}', employee_name)
            message = message.replace('{action}', action)
            return message
        else:
            return f"📍 Tu {action} ya está registrada ✅\n\nAhora puedes enviar tu ubicación para completar el registro.\n\n*Instrucciones:*\n1️⃣ Toca el botón 📎 (clip)\n2️⃣ Selecciona *Ubicación*\n3️⃣ Elige *Ubicación actual*\n4️⃣ Envía tu ubicación\n\n⏰ Tienes 3 minutos para enviarla."

    def get_location_received_message(self, employee_name, latitude, longitude):
        """Genera el mensaje de confirmación de ubicación"""
        maps_url = f"https://maps.google.com/?q={latitude},{longitude}"
        if self.location_received_message:
            message = self.location_received_message
            message = message.replace('{employee_name}', employee_name)
            message = message.replace('{latitude}', str(latitude))
            message = message.replace('{longitude}', str(longitude))
            message = message.replace('{maps_url}', maps_url)
            return message
        else:
            return f"🎉 *¡Ubicación añadida exitosamente!*\n\n📍 Coordenadas: {latitude:.6f}, {longitude:.6f}\n\n🗺️ Ver en Google Maps: {maps_url}\n\n✅ Registro completado"
