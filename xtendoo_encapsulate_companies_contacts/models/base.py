...
class Base(models.AbstractModel):
    ...
    def create(self, vals_list):
        ...
        def get_company_id():
            ...
            try:
                _logger.info(f"[xtendoo_encapsulate_companies_contacts] Fallback usando env.company: {self.env.company.id}")
                return self.env.company.id
            except Exception:
                _logger.exception("[xtendoo_encapsulate_companies_contacts] Error obteniendo company_id desde env.company")
                return False
        ...
        def assign_company(val_dict):
            ...
            _logger.info(f"[xtendoo_encapsulate_companies_contacts] Valor original de company_id en vals: {original_company!r} (tipo: {type(original_company)})")
            ...
            if should_assign:
                ...
                _logger.info(
                    f"[xtendoo_encapsulate_companies_contacts] Asignando company_id={new_company} automáticamente "
                    f"al crear un registro en el modelo {self._name}. Vals: {val_dict}"
                )
            else:
                _logger.info(f"[xtendoo_encapsulate_companies_contacts] No se asigna company_id en modelo {self._name}. Vals: {val_dict}")
        ...
        _logger.info(f"[xtendoo_encapsulate_companies_contacts] Vals final para create en {self._name}: {vals_list}")
        return super(Base, self).create(vals_list)
    ...
    def default_get(self, fields_list):
        ...
        try:
            if 'company_id' in fields_list:
                ...
                _logger.info(f"[xtendoo_encapsulate_companies_contacts] default_get en {self._name}, valor original de company_id: {orig!r}")
                if not orig:
                    ...
                    if company_ctx:
                        ...
                        _logger.info(f"[xtendoo_encapsulate_companies_contacts] default_get ha puesto company_id desde allowed_company_ids: {defaults['company_id']}")
                    elif hasattr(self.env, 'user') and self.env.user and self.env.user.company_id:
                        defaults['company_id'] = self.env.user.company_id.id
                        _logger.info(f"[xtendoo_encapsulate_companies_contacts] default_get ha puesto company_id desde user.company: {defaults['company_id']}")
        except Exception:
            _logger.exception("[xtendoo_encapsulate_companies_contacts] Error en default_get al establecer company_id")
        return defaults
