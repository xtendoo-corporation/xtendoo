...
class multiple_purchase_one_invoice_wizard(models.TransientModel):
    ...
    def create_invoices_for_purchase_order(self):
        ...
        payment_term_id = [order.payment_term_id for order in purchases if order.payment_term_id.id]
        ...
        valid_purchase_orders = purchase_order_obj.search([('partner_id','=',partner_ids[0].id),('state','in',['done','purchase'])])
        valid_purchase_order_ids = map(int, valid_purchase_orders)
        for po in purchases:
            if po.id not in valid_purchase_order_ids or po.invoice_count!=0:
                raise UserError(_("All Purchase Order should be confirmed and uninvoiced"))
