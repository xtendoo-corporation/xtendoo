...
class ResConfigSettings(models.TransientModel):
    ...
    @api.model
    def _get_xtendoo_stock_available_mrp_based_on(self):
    ...
    module_xtendoo_stock_available_immediately = fields.Boolean(
        string="Exclude incoming goods",
        help="This will subtract incoming quantities from the quantities "
        "available to promise.\n"
        "This installs the module xtendoo_stock_available_immediately.",
    )

    module_xtendoo_stock_available_sale = fields.Boolean(
        ...
        "This installs the modules xtendoo_stock_available_sale.\n"
        "If the modules sale and sale_delivery_date are not "
        "installed, this will install them too",
    )

    module_xtendoo_stock_available_mrp = fields.Boolean(
        ...
        "promise.\n"
        "This installs the module xtendoo_stock_available_mrp.\n"
        "If the module mrp is not installed, this will install it "
        "too",
    )

    xtendoo_stock_available_mrp_based_on = fields.Selection(
        _get_xtendoo_stock_available_mrp_based_on,
        string="based on",
    ...
    def get_values(self):
        ...
        res.update(
            xtendoo_stock_available_mrp_based_on=self.env["ir.config_parameter"]
            .sudo()
            .get_param("xtendoo_stock_available_mrp_based_on", "qty_available")
        )
        return res
    ...
    def set_values(self):
        ...
        self.env["ir.config_parameter"].sudo().set_param(
            "xtendoo_stock_available_mrp_based_on", self.xtendoo_stock_available_mrp_based_on
        )
        return res
