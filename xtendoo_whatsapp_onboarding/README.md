...
campañas: esa funcionalidad ya existe en `xtendoo_mail_gateway_whatsapp` y módulos
...
1. Asegúrate de que `xtendoo_mail_gateway_whatsapp` está instalado.
...
- `xtendoo_mail_gateway_whatsapp`: dependencia técnica real. Este módulo escribe
...
instalados en el entorno de pruebas): `xtendoo_mail_gateway_whatsapp_chatter`,
`xtendoo_mail_gateway_whatsapp_variables`, `xtendoo_booking_reserve_whatsapp`. Son
...
envío de mensajes (`xtendoo_mail_gateway_whatsapp._send`, sobrescrito por este
...
`xtendoo_mail_gateway` (`<url-base>/gateway/whatsapp/<webhook_key>/update`). El
...
1. Instalar `xtendoo_mail_gateway_whatsapp`, `xtendoo_mail_gateway_whatsapp_chatter`,
   `xtendoo_mail_gateway_whatsapp_variables`, `xtendoo_booking_reserve_whatsapp` y
...
5. Verificar que el gateway queda en estado "Connected" con WABA ID, Phone
   ...
   (de `xtendoo_mail_gateway_whatsapp`) funciona con el token obtenido.
...
- Mensajería, conversaciones, plantillas, campañas: fuera de alcance,
  ya cubiertas por `xtendoo_mail_gateway_whatsapp` y módulos relacionados.
