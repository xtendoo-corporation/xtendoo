...
class MailGateway(models.Model):
    ...
    # The phone number's two-step verification PIN for the Cloud API,
    # generated by us (the Tech Provider) and sent to Meta when calling
    # POST /{phone_number_id}/register - a required step after Embedded
    # Signup, or the client can never send/receive messages even though
    # the WABA/app connection itself looks complete. Meta exposes no way
    # to retrieve this PIN later (only to reset it via WhatsApp Manager),
    # so this field is its only record.
    whatsapp_onboarding_pin = fields.Char(
        string="WhatsApp 2-Step PIN",
        copy=False,
        default=lambda self: "".join(str(secrets.randbelow(10)) for _ in range(6)),
    )
    ...
    def action_save_meta_credentials(
        ...
        try:
            ...
            # Required by Meta, undocumented in the UI: without this, the
            # client's number stays visible/connected-looking but cannot
            # actually send or receive messages via the Cloud API.
            ...
            self._whatsapp_onboarding_register_phone_number(
                resolved_phone_id, access_token, config, self.whatsapp_onboarding_pin
            )
            _logger.info(
                "[WhatsApp Onboarding] Numero registrado en Cloud API (gateway=%s)",
                self.id,
            )
    ...
    def _whatsapp_onboarding_register_phone_number(
        self, phone_number_id, access_token, config, pin
    ):
        """Registers the phone number for use with the Cloud API.

        Mandatory step after Embedded Signup for a Tech Provider (distinct
        from, and in addition to, subscribing the app to the WABA):
        without it the client's WABA/number look fully connected but
        cannot actually send or receive messages. `pin` is the Tech
        Provider's own choice of 6-digit two-step verification PIN for
        this number - Meta does not generate or return one.
        https://developers.facebook.com/documentation/business-messaging/whatsapp/business-phone-numbers/registration
        """
        endpoint = self._whatsapp_onboarding_graph_url(
            f"{phone_number_id}/register", config
        )
        response = requests.post(
            endpoint,
        ...
        if not payload.get("success"):
            raise UserError(
                _(
                    "Meta refused to register the WhatsApp phone number "
                    "for use with the Cloud API."
                )
            )
        return payload
