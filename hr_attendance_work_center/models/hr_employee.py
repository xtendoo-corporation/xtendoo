...
class HrEmployee(models.Model):
    ...
    def attendance_manual_work_center(self, next_action, work_center_id=None, entered_pin=None):
        print("Metodo work center")
        self.ensure_one()
        ...
        if can_check_without_pin or entered_pin is not None and entered_pin == self.sudo().pin:
            return self._attendance_action_work_center(next_action, work_center_id)
    ...
    def _attendance_action_work_center(self, next_action, work_center_id=None):
        ...
        if employee.user_id:
            modified_attendance = employee.with_user(employee.user_id)._attendance_action_change_work_center(work_center_id)
        else:
            modified_attendance = employee._attendance_action_change_work_center(work_center_id)
        action_message['attendance'] = modified_attendance.read()[0]
        action_message['total_overtime'] = employee.total_overtime
        return {'action': action_message}
    def _attendance_action_change_work_center(self, work_center_id=None):
