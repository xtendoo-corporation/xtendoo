...
class WooQueueLineDashboard(models.AbstractModel):
    ...
    def _prepare_query(self, duration, state, table):
        ...
        if table == 'woo_order_data_queue_line_ept':
            queue_type = next((sublist[2] for sublist in self._context.get('action_domain') if
                               isinstance(sublist, list) and sublist[0] == 'queue_type'), None)
            ...
            qry = """
                    ...
                    INNER JOIN woo_order_data_queue_ept as oq 
                    ON oq.id = oql.order_data_queue_id 
                    AND oq.queue_type = %s 
                    AND oql.state = %s
            """
            if duration == 'today':
