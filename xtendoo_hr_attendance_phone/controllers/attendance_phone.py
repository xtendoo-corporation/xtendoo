...
class AttendancePhone(http.Controller):
    ...
    def attendance_phone_mobile(self):
        ...
        return request.render(
            "xtendoo_hr_attendance_phone.attendance_phone_mobile_page"
        )
    ...
    def set_attendance_settings(self, token, mode, **kwargs):
        ...
        try:
            ...
            if not company:
                _logger.warning(
                    "[ATTENDANCE_PHONE] Token de kiosco inválido: %s", token
                )
            ...
            _logger.info(
                "[ATTENDANCE_PHONE] Modo configurado exitosamente: %s para empresa %s",
                mode,
                company.name,
            )

            return {"success": True, "mode": mode, "company": company.name}
    ...
    @http.route(
        "/attendance/phone/validate_phone_only", type="json", auth="public", csrf=False
    )
    def validate_employee_phone_only(self, phone, token=None, **kwargs):
        ...
        """
        _logger.info(
            "[ATTENDANCE_PHONE_ONLY] Validación solo teléfono - Teléfono: %s***",
            phone[:3] if phone else "N/A",
        )
        ...
        try:
            ...
            _logger.info(
                "[ATTENDANCE_PHONE_ONLY] Teléfono limpiado: %s***",
                phone_cleaned[:3] if len(phone_cleaned) >= 3 else phone_cleaned,
            )
            ...
            for emp in all_employees:
                ...
                mobile_clean = "".join(
                    filter(str.isdigit, (emp.mobile_phone or "").replace("+", ""))
                )
                work_clean = "".join(
                    filter(str.isdigit, (emp.work_phone or "").replace("+", ""))
                )
                ...
                if mobile_clean == phone_cleaned or work_clean == phone_cleaned:
                    employee = emp
                    _logger.info(
                        "[ATTENDANCE_PHONE_ONLY] ✅ Empleado encontrado: %s", emp.name
                    )
                    break

            if not employee:
                _logger.warning(
                    "[ATTENDANCE_PHONE_ONLY] No se encontró empleado con el teléfono"
                )
                return {"success": False, "error": "Teléfono no registrado"}

            _logger.info(
                "[ATTENDANCE_PHONE_ONLY] Empleado encontrado: %s (ID: %s)",
                employee.name,
                employee.id,
            )
            ...
            try:
                ...
                _logger.info(
                    "[ATTENDANCE_PHONE_ONLY] ✅ Asistencia registrada para %s",
                    employee.name,
                )
            except Exception as attendance_error:
                _logger.error(
                    "[ATTENDANCE_PHONE_ONLY] Error registrando asistencia: %s",
                    str(attendance_error),
                )
                raise
    ...
    def validate_employee(self, phone, pin, token=None, **kwargs):
        ...
        _logger.info(
            "[ATTENDANCE_PHONE] Iniciando validación - Teléfono: %s***",
            phone[:3] if phone else "N/A",
        )
        ...
        try:
            if not phone or not pin:
                _logger.warning(
                    "[ATTENDANCE_PHONE] Parámetros faltantes - Phone: %s, Pin: %s",
                    bool(phone),
                    bool(pin),
                )
            ...
            employees_with_pin = (
                request.env["hr.employee"].sudo().search([("pin", "=", pin)])
            )
            _logger.info(
                "[ATTENDANCE_PHONE] Empleados encontrados con PIN %s: %s",
                pin,
                len(employees_with_pin),
            )
            ...
            for emp in employees_with_pin:
                ...
                if mobile_clean == phone_cleaned or work_clean == phone_cleaned:
                    employee = emp
                    _logger.info(
                        "[ATTENDANCE_PHONE] ✅ Coincidencia encontrada con empleado %s",
                        emp.name,
                    )
                    break
            ...
            try:
                ...
                _logger.info(
                    "[ATTENDANCE_PHONE] ✅ Asistencia registrada exitosamente para %s",
                    employee.name,
                )
                ...
                _logger.error(
                    "[ATTENDANCE_PHONE] Error al registrar asistencia: %s",
                    str(attendance_error),
                )
                return {
                    "success": False,
                    "error": f"Error al registrar asistencia: {str(attendance_error)}",
                }
    ...
    def get_employee_status(self, phone, **kwargs):
        ...
        """
        _logger.info(
            "[ATTENDANCE_PHONE_STATUS] Consultando estado - Teléfono: %s***",
            phone[:3] if phone else "N/A",
        )
        ...
        try:
            ...
            phone_cleaned = "".join(filter(str.isdigit, phone.replace("+", "")))
            _logger.info(
                "[ATTENDANCE_PHONE_STATUS] Teléfono limpiado: %s***",
                phone_cleaned[:3] if len(phone_cleaned) >= 3 else phone_cleaned,
            )
            ...
            for emp in all_employees:
                ...
                if mobile_clean == phone_cleaned or work_clean == phone_cleaned:
                    employee = emp
                    _logger.info(
                        "[ATTENDANCE_PHONE_STATUS] ✅ Empleado encontrado: %s", emp.name
                    )
                    break
            ...
            if current_attendance:
                ...
                _logger.info(
                    "[ATTENDANCE_PHONE_STATUS] Empleado DENTRO desde: %s",
                    current_attendance.check_in,
                )
            else:
                status = "FUERA"
    ...
    def register_employee_phone(self, employee_pin, phone_number, **kwargs):
        ...
        try:
            ...
            employee = (
                ...
                .sudo()
                .search([("pin", "=", employee_pin)], limit=1)
            )

            if not employee:
                _logger.warning(
                    "[ATTENDANCE_PHONE] No se encontró empleado con el PIN proporcionado"
                )
            ...
            if existing:
                _logger.warning(
                    "[ATTENDANCE_PHONE] El teléfono ya está registrado para otro empleado"
                )
                return {
                    "success": False,
                    "error": "Este teléfono ya está registrado para otro empleado",
                }
            ...
            if not employee.mobile_phone:
                ...
                _logger.info(
                    "[ATTENDANCE_PHONE] Teléfono registrado en mobile_phone para empleado %s",
                    employee.name,
                )
            elif not employee.work_phone:
                employee.sudo().write({"work_phone": phone_number})
                _logger.info(
                    "[ATTENDANCE_PHONE] Teléfono registrado en work_phone para empleado %s",
                    employee.name,
                )
            else:
                ...
                employee.sudo().write({"mobile_phone": phone_number})
                _logger.info(
                    "[ATTENDANCE_PHONE] Teléfono actualizado en mobile_phone para empleado %s",
                    employee.name,
                )
    ...
    @http.route(
        "/attendance/phone/check-registration", type="json", auth="public", csrf=False
    )
