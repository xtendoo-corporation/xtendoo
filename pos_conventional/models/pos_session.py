...
class PosSession(models.Model):
    ...
    def _compute_linked_sale_orders_total(self):
        ...
        for session in self:
            ...
            session.linked_sale_orders_total = sum(order.amount_total for order in linked_orders)
    ...
    def action_pos_session_closing_control(self, balancing_account=False, amount_to_balance=0,
        ...
        for session in self:
            # Obtener pedidos en borrador que NO están vinculados a sale.order
            draft_orders_not_linked = [
                order for order in session.order_ids
                if order.state == 'draft' and not order.linked_sale_order_id
            ]

            # Solo bloquear si hay pedidos en borrador NO vinculados
            if draft_orders_not_linked:
                raise UserError(_(
            ...
            if session.rescue and session.config_id.cash_control:
                default_cash_payment_method_id = session.payment_method_ids.filtered(lambda pm: pm.type == 'cash')
    ...
    def _check_if_no_draft_orders(self):
        """
        Override para excluir pedidos vinculados a sale.order de la validación de borradores.
        """
        # Obtener solo pedidos en borrador que NO están vinculados a sale.order
        draft_orders = self.get_session_orders().filtered(
            lambda order: order.state == 'draft' and not order.linked_sale_order_id
        )
        if draft_orders:
            raise UserError(_(
    ...
    def _cannot_close_session(self, bank_payment_method_diffs=None):
        ...
        # Solo considerar pedidos en borrador que NO están vinculados a sale.order
        draft_orders_not_linked = [
            order for order in self.get_session_orders()
            if order.state == 'draft' and not order.linked_sale_order_id
        ]
        ...
        if draft_orders_not_linked:
            return {
                'successful': False,
                'message': _("No puede cerrar el TPV si aún hay pedidos en borrador para este día."),
                'redirect': False
            }
