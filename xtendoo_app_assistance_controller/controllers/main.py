...
class XtendooAppAssistanceController(http.Controller):
        ...
        try:
            ...
            if not pin or not telefono:
                return request.make_json_response(
                    {"status": "error", "message": "PIN o teléfono no proporcionados"}
                )
            ...
            if not employee:
                _logger.error(
                    f"Empleado no encontrado con PIN='{pin}' y Teléfono='{telefono}'"
                )
                return request.make_json_response(
    ...
    def get_employee_status(self, **kwargs):
        ...
        try:
            ...
            message = (
                "Empleado actualmente fichado dentro."
                if is_inside
                else "Empleado actualmente fichado fuera."
            )
