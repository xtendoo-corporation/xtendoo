...
class MailThread(models.AbstractModel):
    ...
    def _thread_to_store(self, store: Store, fields, *, request_list=None):
        ...
        for record in self:
            # Get gateway followers directly from message_follower_ids to avoid recursion
            # (message_get_followers calls store.add which calls _thread_to_store again)
            followers = record.message_follower_ids.partner_id
            gateway_followers = []
            for partner in followers:
                gateway_channels = partner.sudo().gateway_channel_ids.mail_format()
                if gateway_channels:
                    gateway_followers.append({
                        "id": partner.id,
                        "name": partner.name,
                        "gateway_channels": gateway_channels,
                    })
            if gateway_followers:
                store.add_model_values(
                    "mail.thread",
                    {
                        "id": record.id,
                        "model": record._name,
                        "gateway_followers": gateway_followers,
                    },
                )
        return res
