...
class ResPartner(models.Model):
    ...
    gateway_channels = fields.Json(compute="_compute_gateway_channels")

    @api.depends("gateway_channel_ids")
    def _compute_gateway_channels(self):
        for partner in self:
            partner.gateway_channels = partner.sudo().gateway_channel_ids.mail_format()

    def _to_store_defaults(self, target):
        """Add gateway_channels to partner data sent to web client."""
        defaults = super()._to_store_defaults(target)
        defaults.append("gateway_channels")
        return defaults

    def _get_channels_as_member(self):
        channels = super()._get_channels_as_member()
        if self.env.user.has_group("xtendoo_mail_gateway.gateway_user"):
...
class ResPartnerGatewayChannel(models.Model):
    ...
    @api.depends_context("xtendoo_mail_gateway_partner_info")
    def _compute_display_name(self):
        # Be able to tell to which partner belongs the gateway partner channel
        # e.g.: picking it from a selector
        if not self.env.context.get("xtendoo_mail_gateway_partner_info"):
