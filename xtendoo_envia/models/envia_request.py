...
# Envia usa códigos de estado personalizados diferentes a los de Odoo.
# Mapa de (country_code, state_code) al código de Envia.
...
class Envia:
    ...
    def _make_api_request(self, endpoint, is_query=False, geolocate=False, method='GET', data=None, params=None):
        """Realiza una llamada a la API de Envia y retorna la respuesta."""
        headers = {
        ...
        try:
            # Envia no maneja correctamente strings UTF-8 según la especificación JSON.
            # Las peticiones con caracteres unicode escapados (\\u) fallan en el lado de Envia.
            # Necesitamos codificar los datos con ensure_ascii=False para enviar con (\x).
            if data:
    ...
    def _envia_parse_error(self, json_data):
        """Devuelve mensajes de error formateados de las peticiones a Envia."""
        error_msgs = []
    ...
    def _get_rate(self, shipper, recipient, packages_data, order):
        """Obtiene la tarifa desde la API de Envia."""
        packages = []
        ...
        if not isinstance(services, list) or not services:
            return {'error_found': _("No se encontró tarifa")}
    ...
    def _rate_request(self, recipient, shipper, order, order_weight=False):
        """Devuelve el diccionario con la tarifa de envío de Envia."""
        if not order:
            raise UserError(_("Se requiere una Orden de Venta para obtener la tarifa."))
        ...
        if bad_products:
            ...
            raise ValidationError(_(
                "Envia Error: Los siguientes productos no tienen peso configurado: %(product_names)s",
    ...
    def _fetch_envia_carriers(self):
        """Importa todos los carriers disponibles de Envia para un país específico."""
        ...
        for international in [0, 1]:
            carrier_json = self._make_api_request(
                f'available-service/{country_code}/{international}/{ship_type}', is_query=True
            )
            if carrier_json.get('statusCode') == 401:
                raise UserError(_(
                    "Envia Error: La API key que ingresaste para %(carrier_name)s parece ser inválida",
    ...
    def _convert_currency(self, amount, picking=False, order=False, from_envia=False):
        """Convierte la moneda de un monto desde o hacia la moneda de Envia."""
        if not (order or picking):
            raise UserError(_('Se requiere una Orden de Venta o Picking para convertir la moneda.'))
    ...
    def _get_shipping_lines(self, package):
        """Devuelve los productos del picking específico para crear la orden."""
        line_by_product = []
        original_prices = []
    ...
    def _prepare_parcel(self, package, origin):
        """Prepara el paquete para el envío del picking."""
        dimensions = dict(package.dimension)
        ...
        if package_type == 'pallet':
            if origin.country_id == origin.env.ref('base.mx'):
                ...
                for comm in package.commodities:
                    ...
                    if not unspsc:
                        msg = _(
                            "Para envíos LTL en México se requiere una Carta Porte. "
                            "Para enviar la información requerida debes configurar el código UNSPSC en "
                            "el siguiente producto: %(product_name)s",
                            product_name=comm.product_id.display_name
                        )
    ...
    def _get_shipping_params(self, picking):
        """Devuelve los parámetros del picking para crear un pedido Envia."""
        ...
        if not package_types or any(package_type != default_package.envia_mail_type for package_type in package_types):
            raise ValidationError(_(
                "Envia Error: El Tipo de Correo Envia (%(package_types)s) configurado en el/los paquete(s) "
                "no coincide con el tipo configurado en el carrier (%(carrier_package_type)s). "
                "Usa un paquete diferente o un carrier que coincida con el tipo de correo.",
    ...
    def _send_shipping(self, picking):
        """Devuelve un diccionario con el precio del envío, números de seguimiento e IDs de órdenes Envia."""
        res = {
            'exact_price': 0.00,
            'tracking_number': '',
        }
        ...
        params = {'label': ','.join(tracking_numbers)}
        tracking_url = self.tracking_url + f"tracking?{urlencode(params)}"
        formatted_tracking = ", ".join(str(t) for t in tracking_numbers)
        ...
        logmessage = Markup(
            ...
            header=_("Envío creado en Envia"),
            tracking_header=_("Números de Seguimiento:"),
        ...
        for label_url, track_number in zip(label_urls, tracking_numbers):
            try:
                ...
                label_prefix = self.carrier._get_delivery_label_prefix()
                label_data.append((
                    '%s-%s.%s' % (label_prefix, track_number, self.carrier.envia_label_file_type),
                    response.content,
                ))
        ...
        if additional_files:
            logmessage = Markup("{header}<br/>").format(header=_("Documentos Envia:"))
            forms_data = []
            doc_prefix = self.carrier._get_delivery_doc_prefix()
            for form_url in additional_files:
                try:
                    response = self.session.get(form_url, timeout=30)
                    response.raise_for_status()
                    forms_data.append((
                        '%s-%s' % (doc_prefix, form_url.split('/')[-1]),
                        response.content,
                    ))
                except (requests.exceptions.HTTPError, requests.exceptions.ReadTimeout, requests.exceptions.ConnectionError):
    ...
    def _cancel_picking(self, picking):
        """Cancela la orden individual en Envia.
        Puede fallar si el envío ya fue despachado.
        Retorna una lista de trackings que no pudieron cancelarse.
        """
        carrier = self.carrier.envia_carrier_code
    ...
    def _get_envia_state_code(self, partner):
        """Envia requiere el código de estado del partner.
        Los códigos que usan son ligeramente distintos a los ISO que usa Odoo.
        """
    ...
    def _get_envia_vat(self, partner):
        """Envia requiere el VAT del partner.
        Si el partner es una dirección de entrega, el VAT está en el parent_id.
        En Colombia, Envia no espera puntos o guiones en el VAT.
        """
    ...
    def _get_envia_city(self, partner):
        # Odoo 17: city_id puede o no estar disponible según el módulo base_address_extended
        city = getattr(partner, 'city_id', False) and partner.city_id.name or partner.city
        return city

    def _get_envia_street(self, partner):
        # Odoo 17: street_name puede o no estar disponible según el módulo base_address_extended
        street = getattr(partner, 'street_name', None) or partner.street
        return street

    def _get_envia_district(self, partner):
        """Envia requiere que la ciudad se envíe dos veces para Chile.
        Para México requiere el campo colonia si está instalado l10n_mx_edi_extended.
        """
        ...
        if l10n_mx.state == 'installed' and hasattr(partner, 'l10n_mx_edi_colony') and partner.l10n_mx_edi_colony:
    ...
    def _geolocate_zip(self, partner):
        """En algunos países el código postal no es obligatorio en el front-end de Envia.
        Si no está configurado en el partner, intentamos geolocalizarlo.
        """
    ...
    def _get_envia_zip(self, partner):
        # Módulo l10n_co_edi almacena el código de estado para Colombia
        l10n_co = partner.env['ir.module.module'].search([('name', '=', 'l10n_co_edi')])

        if l10n_co.state == 'installed' and hasattr(partner, 'city_id') and partner.city_id and partner.city_id.l10n_co_edi_code:
            base_code = str(partner.city_id.l10n_co_edi_code)
            return base_code

        city_zipcode = (hasattr(partner, 'city_id') and partner.city_id and getattr(partner.city_id, 'zipcode', False)) or False
        if not (partner.zip or city_zipcode):
            zipcode = self._geolocate_zip(partner)
            if not zipcode:
                msg = _(
                    "Envia no pudo localizar un código postal para el partner: %(partner_name)s. "
                    "Asegúrate de que la ciudad/municipio y el estado/región estén configurados, "
                    "o ingresa un código postal directamente.",
                    partner_name=partner.display_name
                )
                raise ValidationError(msg)
            return zipcode
        return partner.zip or city_zipcode

    def _get_envia_phone(self, partner):
        """Envia requiere que los números de teléfono estén en formato nacional."""
        phone = partner.phone or partner.mobile
        # Compatibilidad: _phone_format puede no aceptar force_format en todas las versiones
        try:
            phone_formatted = partner._phone_format(number=phone, force_format='NATIONAL') if phone else None
        ...
        if not phone:
            msg = _("Debe configurarse un número de teléfono en el Partner: %(partner_name)s", partner_name=partner.display_name)
            raise ValidationError(msg)
        return phone

    def _prepare_address_values(self, partner, is_cust=False):
        """Prepara el diccionario de dirección requerido para origen/destino en Envia.
        Lanza ValidationError si falta algún campo requerido.
        """
        ...
        address_dict = {
            ...
            'number': (getattr(partner, 'street_number', None) or partner.street2 or ''),
        ...
        for field in country_requirements:
            ...
            if value:
                ...
                missing_fields.append(_(
                    '%(field_name)s debe configurarse en el Partner: %(partner_name)s.',
        ...
        if missing_fields:
            msg = "\n".join(missing_fields)
            raise ValidationError(_('Campos faltantes:\n%s', msg))
