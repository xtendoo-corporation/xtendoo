...
class AccountBankStatement(models.Model):
    ...
    def action_recalculate_running_balance(self):
        """
        Recalcula el running balance para todas las líneas del extracto bancario.
        Este método procesa las líneas ordenadas por internal_index y recalcula
        el balance acumulado.
        """
        for statement in self:
            lines = statement.line_ids.sorted(key=lambda l: (l.internal_index, l.id))
            running_balance = statement.balance_start

            for line in lines:
                running_balance += line.amount
                if line.running_balance != running_balance:
                    line.running_balance = running_balance

            statement.message_post(
                body=_('Running balance recalculado para {} líneas. Balance final: {}').format(
                    len(lines), running_balance
                ),
                subject='Recálculo masivo de running balance'
            )
        ...
        return {
            ...
            'tag': 'display_notification',
            'params': {
                'title': _('Running Balance Recalculated'),
                'message': _('The running balance has been recalculated for all lines.'),
                'type': 'success',
                'sticky': False,
            }
        }
    ...
    def action_recalculate_internal_index(self):
        """
        Recalcula el internal_index para todas las líneas del extracto bancario.
        Este método reordena las líneas según su fecha y secuencia, y asigna
        un internal_index consecutivo comenzando desde 0.
        """
        for statement in self:
            # Ordenar líneas por fecha y luego por secuencia
            lines = statement.line_ids.sorted(key=lambda l: (l.date, l.sequence or 0, l.id))

            # Recalcular el internal_index
            for index, line in enumerate(lines):
                if line.internal_index != index:
                    line.internal_index = index

            statement.message_post(
                body=_('Internal index recalculado para {} líneas.').format(len(lines)),
                subject='Recálculo de internal index'
            )
        ...
        return {
            ...
            'tag': 'display_notification',
            'params': {
                'title': _('Internal Index Recalculated'),
                'message': _('The internal index has been recalculated for all lines.'),
                'type': 'success',
                'sticky': False,
            }
        }
