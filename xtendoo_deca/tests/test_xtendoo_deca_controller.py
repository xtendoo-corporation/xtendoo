...
class TestXtendooDecaController(HttpCase):
    ...
    def setUp(self):
        super().setUp()
        # Cada test crea su propio albarán/DeCA: al pasar por el
        # controlador HTTP, las escrituras se confirman en la base de
        # datos real y no se deshacen entre tests como con TransactionCase,
        # así que compartir un documento a nivel de clase contaminaría
        # otros tests (p. ej. "desactivar descarga" afectaría a los demás).
        cargador = self.env['res.partner'].create({
            'name': 'Cargador HTTP Test', 'vat': 'ES00000001T',
            'street': 'Calle HTTP Test 1', 'city': 'Sevilla', 'zip': '41001'})
        transportista = self.env['res.partner'].create({
            'name': 'Transportista HTTP Test', 'vat': 'ES00000002T',
            'is_transportista': True})
        customer = self.env['res.partner'].create({'name': 'Cliente HTTP Test'})
        ...
        self.doc = self.picking.deca_document_ids
