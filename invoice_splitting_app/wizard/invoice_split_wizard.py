...
class SplitInvoiceWiz(models.TransientModel):
    ...
    split_by_line = fields.Selection(
        [('by_quantity', 'Quantity'), ('by_unit_price', 'Unit Price'), ('by_delete_lines', 'Delete Lines')],
    ...
    def _prepare_account_move_vals(self, invoice):
        return {
    ...
    def _prepare_account_move_line_vals(self, account_move, line):
        return {
            'product_id': line.product_id.id,
            'name': line.name,
            'account_id': line.account_id.id,
            ...
            if self.split_selection == 'invoice_line':
                ...
                if self.split_by_line == 'by_quantity':
                    ...
                    for line in self.split_invoice_ids:
                        ...
                        invoice_line_ids = self.env['account.move.line'].with_context(check_move_validity=False).search(
                            [('id', 'in', line.invoice_line_id.ids)])
                        for inv_line in invoice_line_ids:
                            inv_line.with_context(check_move_validity=False,
                                                  skip_account_move_synchronization=True).update(
                                {'quantity': inv_line.quantity - line.split_qty})
