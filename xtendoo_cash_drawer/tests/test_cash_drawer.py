...
Cobertura:
- _detect_docker_host_ip(): resolución por hostname, por /proc/net/route y fallo total.
- _resolve_url(): URL sin localhost, con localhost + Docker host, sin Docker host.
- CashDrawerController.open_cash_drawer(): sin URL, éxito, con/sin API key,
  ConnectionError, Timeout, hint Docker, reintento con segunda URL candidata.
- pos.config: nuevo campo cash_drawer_auto_open (existencia, default, escritura,
  action_test_cash_drawer con y sin URL).
- res.config.settings: campos relacionados y action_test_cash_drawer delegado.
"""
...
class TestDetectDockerHostIp(TransactionCase):
    """Pruebas para la función auxiliar _detect_docker_host_ip()."""
...
class TestResolveUrl(TransactionCase):
    """Pruebas para la función auxiliar _resolve_url()."""
    ...
    def test_non_localhost_url_returned_unchanged(self):
        ...
        url = "http://192.168.1.10:3210/open"
    ...
    def test_localhost_url_generates_docker_candidate_first(self):
        ...
        url = "http://localhost:3210/open"
        with patch(f"{_CONTROLLER_MODULE}._detect_docker_host_ip", return_value="172.17.0.1"):
            result = _resolve_url(url)
        self.assertEqual(result[0], "http://172.17.0.1:3210/open")
    ...
    def test_127_url_replaces_correctly(self):
        ...
        url = "http://127.0.0.1:3210/open"
        with patch(f"{_CONTROLLER_MODULE}._detect_docker_host_ip", return_value="10.0.2.2"):
            result = _resolve_url(url)
        self.assertEqual(result[0], "http://10.0.2.2:3210/open")
...
# ---------------------------------------------------------------------------
# CashDrawerController.open_cash_drawer
# ---------------------------------------------------------------------------


@tagged("post_install", "-at_install", "xtendoo_cash_drawer")
class TestCashDrawerController(TransactionCase):
    """Pruebas para el endpoint proxy open_cash_drawer."""
    ...
    def test_returns_success_on_200_response(self):
        ...
        with patch(self._patch_target, return_value=self._mock_get(200)):
            result = self.controller.open_cash_drawer(
                url="http://192.168.1.10:3210/open", api_key=""
            )
    ...
    def test_api_key_sent_as_header(self):
        ...
        with patch(self._patch_target, return_value=self._mock_get(200)) as mock_get:
            self.controller.open_cash_drawer(
                url="http://192.168.1.10:3210/open", api_key="supersecret"
            )
    ...
    def test_localhost_error_includes_docker_hint(self):
        ...
        with patch(self._patch_target, side_effect=req.exceptions.ConnectionError("Refused")):
            with patch(f"{_CONTROLLER_MODULE}._detect_docker_host_ip", return_value=None):
                result = self.controller.open_cash_drawer(
                    url="http://127.0.0.1:3210/open", api_key=""
                )
        self.assertFalse(result["success"])
    ...
    def test_tries_docker_candidate_before_original(self):
        ...
        with patch(self._patch_target, side_effect=side_effect_get):
            with patch(
                ...
                self.controller.open_cash_drawer(url="http://127.0.0.1:3210/open", api_key="")

        self.assertGreater(len(call_order), 1)
        self.assertEqual(call_order[0], "http://172.17.0.1:3210/open")
        self.assertEqual(call_order[-1], "http://127.0.0.1:3210/open")
    ...
    def test_non_200_response_still_returns_success(self):
        ...
        # El controlador solo verifica que se obtuvo una respuesta HTTP
        self.assertTrue(result["success"])
        self.assertEqual(result["status_code"], 404)


# ---------------------------------------------------------------------------
# pos.config — cash_drawer_auto_open
# ---------------------------------------------------------------------------


@tagged("post_install", "-at_install", "xtendoo_cash_drawer")
class TestPosConfigCashDrawer(TransactionCase):
    """Pruebas para los campos del cajón en pos.config."""
    ...
    def test_action_test_cash_drawer_returns_client_action(self):
        """action_test_cash_drawer debe retornar una acción cliente correcta."""
        self.pos_config.cash_drawer_open_url = "http://192.168.1.10:3210/open"
        ...
        self.assertEqual(action["params"]["url"], "http://192.168.1.10:3210/open")
        self.assertEqual(action["params"]["api_key"], "mykey")

    def test_action_test_cash_drawer_raises_when_no_url(self):
        """action_test_cash_drawer debe lanzar UserError si no hay URL configurada."""
    ...
    def test_cash_drawer_printer_name_field_exists(self):
        ...
        self.pos_config.cash_drawer_printer_name = "EPSON TM-T20III"
        self.assertEqual(self.pos_config.cash_drawer_printer_name, "EPSON TM-T20III")

    def test_cash_drawer_open_url_field_exists(self):
        """El campo cash_drawer_open_url debe existir y almacenarse."""
        url = "http://192.168.1.10:3210/open"
        self.pos_config.cash_drawer_open_url = url
        self.assertEqual(self.pos_config.cash_drawer_open_url, url)

    def test_multiple_configs_have_independent_auto_open(self):
        """Cada pos.config debe tener su propio valor de cash_drawer_auto_open."""
        config2 = self.env["pos.config"].create(
            {"name": "Test POS Cajón 2", "company_id": self.env.company.id}
        )
        self.pos_config.cash_drawer_auto_open = False
        self.assertFalse(self.pos_config.cash_drawer_auto_open)
        self.assertTrue(config2.cash_drawer_auto_open)
...
class TestResConfigSettingsCashDrawer(TransactionCase):
    ...
    def test_pos_cash_drawer_open_url_related_field_reads_pos_config(self):
        """pos_cash_drawer_open_url debe reflejar el valor de pos.config."""
        url = "http://192.168.1.10:3210/open"
        self.pos_config.cash_drawer_open_url = url
        settings = self._create_settings()
        self.assertEqual(settings.pos_cash_drawer_open_url, url)

    def test_pos_cash_drawer_printer_name_related_field_reads_pos_config(self):
